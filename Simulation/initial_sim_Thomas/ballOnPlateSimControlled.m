...
% Initial Ball Controller States
current_states.x_ball = zeros(4,1); % Assuming 4th order based on report eq 6.1
current_states.y_ball = zeros(4,1);

% Initial Motor States (Assuming resting height T_rest)
% Plant states: [position; velocity]
x_plant1 = [T_rest; 0]; x_plant2 = [T_rest; 0]; x_plant3 = [T_rest; 0];
% Controller states (3 states for the 3rd order motor controller)
x_ctrl1 = zeros(3,1); x_ctrl2 = zeros(3,1); x_ctrl3 = zeros(3,1);

% Initial motor heights and currents
h_new1 = T_rest; h_new2 = T_rest; h_new3 = T_rest;
I1 = 0; I2 = 0; I3 = 0;

h_filt = [T_rest; T_rest; T_rest]; 
...
while toc < t_sim
    ...
    %% Controller update
    % HIGH LEVEL: Ball Position -> Desired Angles 
    % [alpha_ref, beta_ref, current_states] = ballPositionControl(ball_pos, target_pos, current_states, dt);
    [alpha_ref, beta_ref] = PDController(ball_pos, ball_vel, target_pos);

    % INVERSE KINEMATICS: Desired Angles -> Target Motor Positions 
    [h1_ref, h2_ref, h3_ref] = angleToPos(alpha_ref, beta_ref, 0);

    % Update Filter (Memory persists because h_filt is NOT reset here)
    filter_smoothing = 2 * pi * 1 * dt; % Use a slightly higher value for faster response
    h_filt(1) = (1 - filter_smoothing)*h_filt(1) + filter_smoothing*h1_ref;
    h_filt(2) = (1 - filter_smoothing)*h_filt(2) + filter_smoothing*h2_ref;
    h_filt(3) = (1 - filter_smoothing)*h_filt(3) + filter_smoothing*h3_ref;

    % Inner Loop Controller (Current)
    [I1, x_ctrl1] = actuatorController(h_filt(1), h_new1, x_ctrl1, dt);
    [I2, x_ctrl2] = actuatorController(h_filt(2), h_new2, x_ctrl2, dt);
    [I3, x_ctrl3] = actuatorController(h_filt(3), h_new3, x_ctrl3, dt);

    %% Sim update    
    % Actuator update
    [h_new1, x_plant1] = actuatorDynamics(I1, x_plant1, dt);
    [h_new2, x_plant2] = actuatorDynamics(I2, x_plant2, dt);
    [h_new3, x_plant3] = actuatorDynamics(I3, x_plant3, dt);

    % 2. Calculate Angles using function
    [alpha, beta, psi] = posToAngle(h_new1, h_new2, h_new3);
    ...
    pos1 = h_new1 - T_rest; % Relative offset for graphics
    pos2 = h_new2 - T_rest;
    pos3 = h_new3 - T_rest;
    ...
    corners_global = (R_total * corners_local) + [0; 0; z_center];
    ...
    % 6. Update plate
    set(hPlate, 'Vertices', corners_global');
